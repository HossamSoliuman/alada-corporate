<a href="{{ route('case-studies.show', $cs->slug) }}"
   class="group block card-glass rounded-2xl overflow-hidden transition-all duration-500 reveal flex flex-col">
    ...
    <div class="p-6 flex-1 flex flex-col">
        <div class="flex flex-wrap gap-2 mb-3">
            @if($cs->industry)<span class="text-xs font-semibold text-brown-300 uppercase tracking-widest">{{ $cs->industry->name }}</span>@endif
            @if($cs->category)<span class="text-xs text-slate-400">· {{ $cs->category->name }}</span>@endif
        </div>
        <h3 class="font-heading text-lg text-white group-hover:text-brown-300 transition-colors leading-snug flex-1">{{ $cs->title }}</h3>
        @if($cs->client_name)<p class="mt-2 text-xs text-slate-400">{{ $cs->client_name }}</p>@endif
        <div class="mt-4 flex items-center gap-2 text-xs font-semibold text-brown-300 group-hover:gap-3 transition-all">
