<header id="site-header" class="bg-white/95 backdrop-blur-sm sticky top-0 z-50 border-b border-slate-200/60" x-data="{ mobileOpen: false, servicesOpen: false }">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-20 transition-all duration-300">
            ...
            <nav class="hidden lg:flex items-center gap-8">
                <a href="{{ route('home') }}" class="text-sm font-medium text-navy-700 hover:text-teal-600 transition-colors tracking-wide">Home</a>
                <a href="{{ route('about') }}" class="text-sm font-medium text-navy-700 hover:text-teal-600 transition-colors tracking-wide">About</a>
                        ...
                        <div class="grid grid-cols-2 gap-1">
                            ...
                            <a href="{{ route('services.show', $svc->slug) }}"
                               class="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-slate-50 transition-colors group/item">
                                <div class="w-8 h-8 rounded-lg bg-teal-50 flex items-center justify-center text-teal-600 group-hover/item:bg-teal-600 group-hover/item:text-white transition-all shrink-0">
                                    <x-icon name="{{ $svc->icon ?? 'building-office-2' }}" class="w-4 h-4"/>
                                </div>
                                <span class="text-xs font-medium text-navy-700 leading-tight">{{ $svc->name }}</span>
                            </a>
                            @endforeach
                        </div>
                        <div class="mt-3 pt-3 border-t border-slate-100">
                            <a href="{{ route('services.index') }}" class="flex items-center gap-2 text-xs font-semibold text-teal-600 hover:text-teal-700 transition-colors">
                                View all services
