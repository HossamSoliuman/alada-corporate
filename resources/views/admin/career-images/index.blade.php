{{-- admin/career-images/index.blade.php --}}
@extends('layouts.admin')
@section('title', 'Career Gallery')
@section('content')

<div class="flex items-center justify-between mb-6">
    <h2 class="text-xl font-heading font-bold text-gray-900">Career Gallery</h2>
    <span class="text-sm text-gray-500">{{ $images->count() }} image(s)</span>
</div>

{{-- Upload --}}
<div class="bg-white rounded-xl border border-gray-200 p-6 mb-6">
    <h3 class="text-sm font-semibold text-gray-700 mb-4">Upload Images</h3>
    <form method="POST" action="{{ route('admin.career-images.store') }}" enctype="multipart/form-data">
        @csrf
...
<div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden group">
        ...
        <div class="p-3 space-y-2">
            <form method="POST" action="{{ route('admin.career-images.update', $image->id) }}">
                @csrf @method('PUT')
                <div class="flex gap-1.5">
                    <input type="number" name="order" value="{{ $image->order }}" min="0"
            ...
            <form method="POST" action="{{ route('admin.career-images.destroy', $image->id) }}"
...
<div class="text-center py-20 bg-white rounded-xl border border-gray-200">
    <p class="text-gray-400 text-sm">No images yet. Upload some above.</p>
</div>
@endif
